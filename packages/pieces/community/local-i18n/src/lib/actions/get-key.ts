...
async function executeGetKey(
  ...
  if (!initialized) {
    return {
      ...
      path: path,
      translation: null,
      error: 'Translation store not initialized. Please run "Set Store" action first.'
    };
  }
  ...
  if (!languagesStr) {
    throw new Error('No languages found in store. Please run "Set Store" action first.');
  }
  ...
  if (!availableLanguages.includes(language)) {
    return {
      ...
      path: path,
      translation: null,
      error: `Language "${language}" not found in store. Available languages: ${availableLanguages.join(', ')}`
    };
  }
  ...
  if (namespaces.length === 0) {
    return {
      ...
      path: path,
      translation: null,
      error: `No resources found for language: ${language}`
    };
  }
...
export const getKeyAction = createAction({
  name: 'get_key',
  displayName: 'Get Key',
  ...
  async run(context) {
    return await executeGetKey(context);
  },
});
