...
import { PieceAuthProperty, StoreScope } from '@activepieces/pieces-framework';
...
>(
  ...
  return {
    ...
    step: { name: 'test-step' },
    store: {
      get: jest.fn(async <T>(key: string, scope?: StoreScope): Promise<T | null> => {
        const val = storeMap.get(key);
        if (val === undefined) return null;
        return val as T;
      }),
      put: jest.fn(async <T>(key: string, value: T, scope?: StoreScope): Promise<T> => {
        const strValue = typeof value === 'string' ? value : JSON.stringify(value);
        storeMap.set(key, strValue);
        return value;
      }),
      delete: jest.fn(async (key: string, scope?: StoreScope): Promise<void> => {
        storeMap.delete(key);
      }),
    },
    flows: {
